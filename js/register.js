...
createAccount.addEventListener('click', (event) => {
    ...
    if ((!username.trim() || !password.trim()) || (username.length > 8)) {
        ...
        if (exists) {
            ...
            users.push({ username: `${username.trim()}`, password: `${password.trim()}`, numberguessing: {win: 0, lose: 0, tg: 0}, rockpaperscissor: {win: 0, lose: 0, tg: 0}, fastclicker: {hs: 0, tg: 0}, pokemonmemorygame: {win: 0, lose: 0, tg: 0}, chickengeprekgame: {hs: 0, tg: 0} });
            saveUsers(users);
